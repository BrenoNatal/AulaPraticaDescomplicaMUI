...
const ListarTarefa = () => {
  ...
  const handleDeletar = (id) => {
    ...
    <Box sx={{ flexGrow: 1, p: 1}} >
      <Grid container sx={{display: 'grid', gap: 1, gridTemplateColumns: 'repeat(3, 1fr)'}}>
        {tarefas.map((row, indice) => (

          <Grid>
          <Card variant="outlined" size>
            <CardContent>
              <Typography>
                {row.tituloTarefa}
              </Typography>
              ...
              <Typography>
              {row.inicioTarefa} - {row.fimTarefa}
              </Typography>
              <Grid container sx={{display: 'grid', gap: 1, gridTemplateColumns: 'repeat(6, 1fr)', mt:1}}>
                <Button variant="contained" color="success" onClick={() => handleEditar(row.idTarefa)}><EditIcon fontSize="small" /></Button>
                <Button variant="contained" color="error" onClick={() => handleDeletar(row.idTarefa)}><DeleteIcon fontSize="small" /></Button>
              </Grid>
            </CardContent>
